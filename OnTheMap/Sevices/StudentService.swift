...
class StudentService: GenericService {
    ...
    func postStudentLocation(student:StudentInformation, completion: @escaping (ResultType<PostStudentResponse>) -> Void) {
        ...
        //I just used this here because  to of some reason my generic service don't parse float values:
        /*
         let parameters = ["latitude":"\(latitude)","uniqueKey":uniqueKey,"firstName":firstName,"lastName":lastName,"mapString":mapString,"mediaURL":mediaURL,"longitude":"\(longitude)"] as [String : Any]
         requestData(type: PostStudentResponse.self, url: UdacityUtils.shared.parseURL, method: .post, parameters: parameters,hasHTTPHeader:false, isParseRequest: true){ completionHandler in
